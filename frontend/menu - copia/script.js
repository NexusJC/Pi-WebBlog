...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function cargarComentarios(mostrarTodos = false) {
    ...
    visibles.forEach(texto => {
      const div = crearComentario(texto);
    ...
    if (comentarios.length > MAX_VISIBLES) {
      ...
      verMasBtn.onclick = () => {
        cargarComentarios(!mostrarTodos); // alterna entre true y false
      };
    } else {
  ...
  function guardarComentario() {
    ...
    if (texto === "") return;
    const datos = JSON.parse(localStorage.getItem("comentariosMenu")) || [];
    datos.push(texto);
    localStorage.setItem("comentariosMenu", JSON.stringify(datos));
    input.value = "";
    cargarComentarios();
  }

  button.addEventListener("click", guardarComentario);
  ...
  // Cargar los comentarios al entrar
  cargarComentarios();
});
