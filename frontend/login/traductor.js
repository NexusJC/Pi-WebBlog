    ...
    for (const nodo of el.childNodes) {
      if (nodo.nodeType === Node.TEXT_NODE && nodo.nodeValue.trim().length > 0) {
        ...
        try {
          ...
          console.warn("Error traduciendo nodo:", textoOriginal, error);
        }
      }
    }

    // Traducción de atributos comunes
    const atributos = ["placeholder", "title", "alt", "value", "aria-label"];
    for (const attr of atributos) {
      if (el.hasAttribute(attr)) {
        const texto = el.getAttribute(attr).trim();
        if (texto) {
          try {
            const textoTraducido = await traducirTexto(texto, idiomaDestino);
            el.setAttribute(attr, textoTraducido);
          } catch (error) {
            console.warn(`Error traduciendo atributo [${attr}]:`, texto, error);
          }
        }
      }
    }
